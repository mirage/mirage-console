...
module Connection = struct
  ...
  let to_assoc_list t =
    ...
    let frontend_acl =
      Xs_protocol.ACL.({owner = t.frontend_domid; other = NONE; acl = [ t.backend_domid, READ ]}) in
    let backend_acl =
      Xs_protocol.ACL.({owner = t.backend_domid; other = NONE; acl = [ t.frontend_domid, READ ]}) in
    List.map (fun (owner, key, value) -> match owner with
